...
if not CLOUD_MONGODB_URI:
    print("❌ Error: MONGODB_URI not found in environment variables")
    print("Please make sure you have:")
    print("1. Created a .env file (copy from .env.example)")
    print("2. Set up your MongoDB Atlas cluster")
    print("3. Added your connection string to MONGODB_URI in .env")
    sys.exit(1)
...
def connect_to_cloud_db():
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        client = MongoClient(
                            modified_uri, serverSelectionTimeoutMS=15000
                        )
                        client.admin.command("ping")
        ...
        print(
            f"python3 -c \"from pymongo import MongoClient; print('✅ OK:', MongoClient('{CLOUD_MONGODB_URI}').admin.command('ping'))\""
        )
        raise
...
def create_users_collection(db):
    ...
    try:
        # Create indexes
        users.create_index(
            [("username", ASCENDING)], unique=True, name="username_unique"
        )
...
def create_locations_collection(db):
    ...
    try:
        ...
        locations.create_index(
            [("zipCode", ASCENDING)], unique=True, name="zipcode_unique"
        )
        ...
        locations.create_index(
            [("city", ASCENDING), ("state", ASCENDING)], name="city_state_idx"
        )
        print("✅ Created compound index on city+state")

        locations.create_index(
            [("state", ASCENDING), ("city", ASCENDING)], name="state_city_idx"
        )
        ...
        try:
            locations.create_index(
                [("city", TEXT), ("stateName", TEXT), ("zipCode", TEXT)],
...
def create_ride_posts_collection(db):
    ...
    try:
        ...
        ride_posts.create_index(
            [("status", ASCENDING), ("travelDate", ASCENDING)], name="status_date_idx"
        )
        ...
        ride_posts.create_index(
            [("seatsRemaining", ASCENDING)], name="seats_remaining_idx"
        )
...
def create_ride_interests_collection(db):
    ...
    try:
        ...
        ride_interests.create_index(
            [("interestedUserId", ASCENDING)], name="interested_user_id_idx"
        )
        print("✅ Created index on interestedUserId")

        # Unique constraint to prevent duplicate interests
        ride_interests.create_index(
            [("rideId", ASCENDING), ("interestedUserId", ASCENDING)],
            unique=True,
            name="ride_user_unique",
        )
        ...
        ride_interests.create_index(
            [("createdAt", DESCENDING)], name="created_at_desc_idx"
        )
        print("✅ Created index on createdAt")

        # Compound indexes for common queries
        ride_interests.create_index(
            [("interestedUserId", ASCENDING), ("status", ASCENDING)],
            name="user_status_idx",
        )
        print("✅ Created compound index on interestedUserId+status")

        ride_interests.create_index(
            [("rideId", ASCENDING), ("status", ASCENDING)], name="ride_status_idx"
        )
...
def create_roommate_posts_collection(db):
    ...
    try:
        ...
        posts.create_index(
            [("status", ASCENDING), ("type", ASCENDING), ("location", ASCENDING)],
            name="status_type_location_idx",
        )

        posts.create_index(
            [("status", ASCENDING), ("moveInEarliest", ASCENDING)],
...
def create_roommate_interests_collection(db):
    ...
    try:
        ...
        interests.create_index(
            [("postId", ASCENDING), ("interestedUserId", ASCENDING)],
            unique=True,
        ...
        interests.create_index(
            [("postId", ASCENDING), ("status", ASCENDING)], name="post_status_idx"
        )
    except OperationFailure as e:
        if "already exists" not in str(e):
...
def create_notifications_collection(db):
    ...
    try:
        ...
        # Compound indexes for common queries
        notifications.create_index(
            [("userId", ASCENDING), ("read", ASCENDING)], name="user_read_idx"
        )
        print("✅ Created compound index on userId+read")

        notifications.create_index(
            [("userId", ASCENDING), ("createdAt", DESCENDING)], name="user_created_idx"
        )
...
def load_locations_from_csv(db, csv_file_path="data/locations.csv"):
    ...
    try:
        ...
        if not os.path.exists(csv_full_path):
            ...
            print(
                "Please ensure your locations.csv file is in the backend/data/ directory."
            )
            return False
...
def main():
    ...
    try:
        ...
        if existing_collections:
            ...
            response = (
                input("Do you want to proceed? This will update/add indexes (y/N): ")
                .strip()
                .lower()
            )
